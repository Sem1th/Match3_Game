...
namespace Match3 {
    public class GridSystem2D<T>{
        ...
        Sprite cellGrid;
        T[,] gridArray;
        public Sprite CellGrid { get { return cellGrid; } } 
        ...
        public static GridSystem2D<T> VerticalGrid(int width, int height, float cellSize, Vector3 origin, Sprite cellGrid, bool debug = false) {
            return new GridSystem2D<T>(width, height, cellSize, origin, cellGrid, new VerticalConverter(), debug);
        }

        public static GridSystem2D<T> HorizontalGrid(int width, int height, float cellSize, Vector3 origin, Sprite cellGrid, bool debug = false) {
            return new GridSystem2D<T>(width, height, cellSize, origin, cellGrid, new HorizontalConverter(), debug);
        }

        public GridSystem2D(int width, int height, float cellSize, Vector3 origin, Sprite cellGrid, CoordinateConverter coordinateConverter, bool debug) {
            this.width = width;
            this.height = height;
        ...
        public void DrowCellSprite() {
            var parent = new GameObject("Sprites");

            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    CreateCellSprite(parent, GetWorldPositionCenter(x, y), cellGrid);
                }
            }
        }

        void CreateCellSprite(GameObject parent, Vector3 position, Sprite sprite) {
            GameObject cellGameObject = new GameObject("CellGrid", typeof(SpriteRenderer));
            cellGameObject.transform.SetParent(parent.transform);
            cellGameObject.transform.position = position;
            cellGameObject.GetComponent<SpriteRenderer>().sprite = sprite;
            cellGameObject.GetComponent<SpriteRenderer>().sortingOrder = -1;
        }
        ...
        TextMeshPro CreateWorldText(GameObject parent, string text, Vector3 position, Vector3 dir,
            ...
            {
                GameObject gameObject = new GameObject("DebugText" + text, typeof(TextMeshPro));
        ...
        /// <summary>
        ///  <Преобразователь координат для вертикальных сеток, где сетка лежит в плоскости X-Y>
        /// </summary>
        public class VerticalConverter : CoordinateConverter {
            public override Vector3 GridToWorld(int x, int y, float cellSize, Vector3 origin) {
                return new Vector3(x, y, 9) * cellSize + origin;
            }
